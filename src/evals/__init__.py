...
def get_evaluator(name: str, eval_cfg: DictConfig, **kwargs):
    ...
    if evaluator_handler_name == "LMEvalEvaluator":
        try:
            from evals.lm_eval import LMEvalEvaluator
        except ModuleNotFoundError as exc:
            if exc.name == "lm_eval" or exc.name.startswith("lm_eval."):
                raise ModuleNotFoundError(
                    "LMEvalEvaluator requires the optional lm-eval dependency. "
                    'Install it with `uv pip install ".[lm-eval]"`.'
                ) from exc
            raise
